    ...
    if [ -f "$fish_state" ] || dpkg -l fish &>/dev/null; then
        if confirm "Fish Shell detectado. Desinstalar?"; then
            ...
            if [ -f "$fisher_state" ] || command -v fisher &>/dev/null; then
                rm -rf ~/.config/fish/completions/fisher.fish ~/.config/fish/conf.d/fisher.fish ~/.config/fish/functions/fisher.fish 2>/dev/null || true
...
mise_installer() {
    ...
    if [ -f "$state_file" ] || command -v mise &>/dev/null; then
        if confirm "Mise detectado. Desinstalar?"; then
            ...
            sed -i '/mise init/d' ~/.bashrc ~/.zshrc ~/.config/fish/config.fish 2>/dev/null || true
            cleanup_files "$state_file" "$HOME/.local/share/mise"
...
repos_nonfree_installer() {
    ...
    if [ -f "$state_file" ]; then
        ...
        if confirm "Adicionar repositórios non-free e contrib?"; then
            echo "Adicionando repositórios non-free e contrib..."
            ...
            sudo apt update
            touch "$state_file"
            echo "Repositórios non-free e contrib adicionados."
        fi
    fi
}
...
starship_installer() {
    ...
    if [ -f "$state_file" ] || command -v starship &>/dev/null; then
        if confirm "Starship detectado. Desinstalar?"; then
            ...
            rm -f /usr/local/bin/starship 2>/dev/null || true
            cleanup_files "$state_file"
...
zswap_installer() {
    ...
    if [ -f "$state_file" ]; then
        if confirm "ZSWAP detectado. Desativar?"; then
            ...
            sudo sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT=".*zswap.enabled=[0-9].*"/GRUB_CMDLINE_LINUX_DEFAULT="quiet"/' /etc/default/grub
            sudo update-grub
            cleanup_files "$state_file"
...
main_menu() {
    while true; do
        ...
        echo "4) Archiving/Compression Tools"
        echo "5) aria2"
        echo "6) CachyOS Configs"
        echo "7) curl"
        echo "8) DebMultimedia Repository"
        echo "9) GNOME Desktop"
        ...
        echo "17) Nala"
        echo "18) NeoVim"
        echo "19) Nvidia Proprietary Driver"
        echo "20) Oh My Bash"
        echo "21) Pacstall"
        echo "22) Personal Base Packages"
        echo "23) Personal Media Packages"
        echo "24) Plasma Desktop"
        echo "25) Podman"
        echo "26) Repositories non-free/contrib"
