...
[ -f /etc/arch-release ] && { echo "Este script é apenas para Debian."; exit 1; }
...
system_update() {
    echo "Atualizando sistema e instalando ntpsec..."
    sudo apt update
    sudo apt upgrade -y
...
apparmor_installer() {
    ...
    local pkg_apparmor="apparmor apparmor-profiles apparmor-utils"
    ...
    if [ -f "$state_file" ] || dpkg -l apparmor &>/dev/null; then
        if confirm "AppArmor detectado. Desinstalar?"; then
            ...
            sudo apt remove -y $pkg_apparmor
            sudo sed -i 's/ apparmor=1 security=apparmor//g' /etc/default/grub
            sudo update-grub
...
archiving_compression_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l p7zip-full &>/dev/null; then
        if confirm "Pacotes de Compactação detectados. Desinstalar?"; then
            ...
            sudo apt remove -y $pkg_compactacao
            cleanup_files "$state_file"
...
debian_repositories_installer() {
    system_update
    local state_file="$STATE_DIR/debian_repos"
    local os_codename=$(grep VERSION_CODENAME /etc/os-release | cut -d= -f2)

    if [ -f "$state_file" ]; then
        if confirm "Repositórios Debian extras detectados. Remover?"; then
            echo "Removendo repositórios extras..."
            sudo rm -f /etc/apt/sources.list.d/debian-multimedia.sources
            sudo rm -f /etc/apt/sources.list.d/nvidia.sources
            sudo sed -i '/contrib non-free non-free-firmware/d' /etc/apt/sources.list
            sudo apt update
            cleanup_files "$state_file"
            echo "Repositórios extras removidos."
        fi
    else
        if confirm "Adicionar repositórios non-free, contrib, non-free-firmware e Debian Multimedia?"; then
            echo "Adicionando repositórios..."
            
            sudo sed -i "s/ $os_codename main/ $os_codename main contrib non-free non-free-firmware/g" /etc/apt/sources.list
            sudo sed -i "s/ $os_codename-updates main/ $os_codename-updates main contrib non-free non-free-firmware/g" /etc/apt/sources.list
            sudo sed -i "s/ $os_codename-security main/ $os_codename-security main contrib non-free non-free-firmware/g" /etc/apt/sources.list
            
            curl -s -o /tmp/deb-multimedia-keyring.deb https://www.deb-multimedia.org/pool/main/d/deb-multimedia-keyring/deb-multimedia-keyring_2024.9.1_all.deb
            sudo dpkg -i /tmp/deb-multimedia-keyring.deb
            rm -f /tmp/deb-multimedia-keyring.deb
            
            cat << EOF | sudo tee /etc/apt/sources.list.d/debian-multimedia.sources
Types: deb
...
gamemode_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l gamemode &>/dev/null; then
        if confirm "Gamemode detectado. Desinstalar?"; then
            ...
            sudo apt remove -y $pkg_gamemode
            cleanup_files "$state_file"
...
mise_installer() {
    ...
    if [ -f "$state_file" ] || command -v mise &>/dev/null; then
        if confirm "Mise detectado. Desinstalar?"; then
            ...
            rm -f ~/.local/bin/mise
            sed -i '/mise/d' ~/.bashrc ~/.zshrc ~/.config/fish/config.fish 2>/dev/null || true
            cleanup_files "$state_file"
        ...
        if confirm "Instalar Mise?"; then
            ...
            if [ -f ~/.bashrc ]; then
                curl -s https://mise.run/bash | sh
                ~/.local/bin/mise use -g usage
                mkdir -p ~/.local/share/bash-completion/completions/
                ~/.local/bin/mise completion bash > ~/.local/share/bash-completion/completions/mise
            fi
            if [ -f ~/.zshrc ]; then
                curl -s https://mise.run/zsh | sh
                ~/.local/bin/mise use -g usage
                sudo mkdir -p /usr/local/share/zsh/site-functions
                ~/.local/bin/mise completion zsh | sudo tee /usr/local/share/zsh/site-functions/_mise
            fi
            if [ -f ~/.config/fish/config.fish ]; then
                curl -s https://mise.run/fish | sh
                ~/.local/bin/mise use -g usage
...
nvidia_proprietary_dkms_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l nvidia-driver &>/dev/null; then
        if confirm "Nvidia Proprietário detectado. Desinstalar?"; then
            ...
            sudo apt remove -y nvidia-driver cuda-drivers nvidia-open cuda-toolkit
        ...
        if confirm "Instalar Nvidia Proprietário?"; then
            ...
            sudo apt install -y --no-install-recommends dkms libdw-dev clang lld llvm build-essential linux-headers-amd64 pipewire-audio-client-libraries
            
            curl -s -o /tmp/cuda-keyring.deb https://developer.download.nvidia.com/compute/cuda/repos/debian12/x86_64/cuda-keyring_1.1-1_all.deb
            sudo dpkg -i /tmp/cuda-keyring.deb
            rm -f /tmp/cuda-keyring.deb
            
            cat << EOF | sudo tee /etc/apt/preferences.d/nvidia-repo
Package: *
...
EOF
            ...
            if confirm "Instalar apenas driver NVIDIA (jogos) ou CUDA completo (desenvolvimento)?"; then
                echo "Instalando CUDA completo..."
...
starship_installer() {
    ...
    if [ -f "$state_file" ] || command -v starship &>/dev/null; then
        if confirm "Starship detectado. Desinstalar?"; then
            ...
            sudo apt remove -y $pkg_starship 2>/dev/null || curl -s https://starship.rs/install.sh | sh -s -- -y -f --uninstall
        ...
        if confirm "Instalar Starship?"; then
            ...
            if sudo apt install -y $pkg_starship 2>/dev/null; then
                true
            else
            ...
            command -v fish &>/dev/null && mkdir -p ~/.config/fish && echo -e "\nstarship init fish | source" >> ~/.config/fish/config.fish
            touch "$state_file"
...
zswap_zram_installer() {
    system_update
    local state_file="$STATE_DIR/zswap_zram"
    local os_codename=$(grep VERSION_CODENAME /etc/os-release | cut -d= -f2)

    if [ -f "$state_file" ]; then
        if confirm "ZSWAP/ZRAM detectado. Desativar?"; then
            echo "Desativando ZSWAP/ZRAM..."
            ...
            cleanup_files "$state_file"
            echo "ZSWAP/ZRAM desativado. Reinicie para aplicar."
        fi
    else
        if confirm "Ativar ZSWAP ou ZRAM? (s=ZSWAP, n=ZRAM)"; then
            echo "Ativando ZSWAP..."
...
main_menu() {
    while true; do
        ...
        echo "3) Archiving/Compression"
        echo "4) Aria2"
        echo "5) Curl"
        echo "6) Debian Repositories (non-free, contrib, multimedia)"
        ...
        echo "15) Nala"
        echo "16) NeoVim"
        echo "17) Nvidia Proprietary"
        echo "18) Pacstall"
        echo "19) Pessoal Base (Fonts)"
        echo "20) Pessoal Media"
        echo "21) Plasma DE"
        echo "22) Podman"
        echo "23) Shader Booster"
        echo "24) Snapd"
        echo "25) Starship"
        echo "26) Steam"
        echo "27) UFW Firewall"
        echo "28) Unmojang (Fjord Launcher)"
        echo "29) XDG Base"
        echo "30) YT-DLP"
        echo "31) Zen Browser"
        echo "32) ZSWAP/ZRAM"
        ...
        case $opcao in
            ...
            4) aria2_installer ;;
            5) curl_installer ;;
            6) debian_repositories_installer ;;
            ...
            15) nala_installer ;;
            16) neovim_installer ;;
            17) nvidia_proprietary_dkms_installer ;;
            18) pacstall_installer ;;
            19) pessoal_base_installer ;;
            20) pessoal_media_installer ;;
            21) de_plasma_installer ;;
            22) podman_installer ;;
            23) shader_booster_installer ;;
            24) snapd_installer ;;
            25) starship_installer ;;
            26) steam_installer ;;
            27) ufw_installer ;;
            28) unmojang_installer ;;
            29) xdg_base_installer ;;
            30) yt_dlp_installer ;;
            31) zen_browser_installer ;;
            32) zswap_zram_installer ;;
            0) exit 0 ;;
            *) echo "Opção inválida" ;;
        esac
