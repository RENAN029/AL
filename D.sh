...
add_contrib_nonfree_repos() {
    ...
    if [ -f "$state_file" ]; then
        if confirm "Repositórios CONTRIB/NON-FREE detectados. Remover?"; then
            ...
            sudo sed -i '/non-free\|contrib/d' /etc/apt/sources.list.d/* 2>/dev/null || true
            sudo apt update
...
apparmor_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l apparmor &>/dev/null; then
        if confirm "AppArmor detectado. Desinstalar?"; then
            ...
            dpkg -l apparmor &>/dev/null && sudo apt purge -y $pkg_apparmor
            cleanup_files "$state_file"
...
appimage_fuse_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l libfuse2 &>/dev/null; then
        if confirm "FUSE para AppImage detectado. Desinstalar?"; then
            ...
            dpkg -l libfuse2 &>/dev/null && sudo apt purge -y $pkg_fuse
            cleanup_files "$state_file"
...
archiving_compression_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l p7zip-full &>/dev/null; then
        if confirm "Pacotes de Compactação detectados. Desinstalar?"; then
            ...
            dpkg -l p7zip-full &>/dev/null && sudo apt purge -y $pkg_compactacao
            cleanup_files "$state_file"
...
aria2_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l aria2 &>/dev/null; then
        if confirm "aria2 detectado. Desinstalar?"; then
            ...
            dpkg -l aria2 &>/dev/null && sudo apt purge -y $pkg_aria2
            cleanup_files "$state_file"
...
curl_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l curl &>/dev/null; then
        if confirm "curl detectado. Desinstalar?"; then
            ...
            dpkg -l curl &>/dev/null && sudo apt purge -y $pkg_curl
            cleanup_files "$state_file"
...
deb_multimedia_installer() {
    ...
    if [ -f "$state_file" ]; then
        if confirm "Repositório DebMultimedia detectado. Remover?"; then
            ...
            sudo rm -f /etc/apt/sources.list.d/dmo.sources /usr/share/keyrings/deb-multimedia-keyring.gpg
            sudo apt update
            cleanup_files "$state_file"
...
de_gnome_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l gnome-shell &>/dev/null; then
        if confirm "Gnome detectado. Desinstalar?"; then
            ...
            dpkg -l gnome-shell &>/dev/null && sudo apt purge -y $pkg_gnome
            cleanup_files "$state_file"
        ...
        if confirm "Instalar Gnome?"; then
            ...
            sudo apt install -y $pkg_gnome gdm3
...
de_plasma_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l plasma-desktop &>/dev/null; then
        if confirm "Plasma detectado. Desinstalar?"; then
            ...
            dpkg -l plasma-desktop &>/dev/null && sudo apt purge -y $pkg_plasma
            cleanup_files "$state_file"
        ...
        if confirm "Instalar Plasma?"; then
            ...
            sudo apt install -y $pkg_plasma sddm
...
fish_fisher_installer() {
    ...
    if [ -f "$fish_state" ] || dpkg -l fish &>/dev/null; then
        if confirm "Fish Shell detectado. Desinstalar?"; then
            ...
            dpkg -l fish &>/dev/null && sudo apt purge -y $pkg_fish
    ...
    if [ -f "$fisher_state" ]; then
        ...
        if command -v fish >/dev/null 2>&1; then
            ...
            echo "Fish Shell não encontrado. Instale-o primeiro."
        fi
    fi
}
...
flatpak_flathub_installer() {
    ...
    if [ -f "$flatpak_state" ] || dpkg -l flatpak &>/dev/null; then
        if confirm "Flatpak detectado. Desinstalar?"; then
            ...
            dpkg -l flatpak &>/dev/null && sudo apt purge -y $pkg_flatpak
...
fwupd_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l fwupd &>/dev/null; then
        if confirm "Fwupd detectado. Desinstalar?"; then
            ...
            dpkg -l fwupd &>/dev/null && sudo apt purge -y $pkg_fwupd
            cleanup_files "$state_file"
...
gamemode_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l gamemode &>/dev/null; then
        if confirm "Gamemode detectado. Desinstalar?"; then
            ...
            dpkg -l gamemode &>/dev/null && sudo apt purge -y $pkg_gamemode
            cleanup_files "$state_file"
...
mise_installer() {
    ...
    if [ -f "$state_file" ]; then
        if confirm "Mise detectado. Desinstalar?"; then
            ...
            rm -f ~/.local/share/bash-completion/completions/mise ~/.config/fish/completions/mise.fish /usr/local/share/zsh/site-functions/_mise 2>/dev/null || true
            sed -i '/mise shellenv/d' ~/.bashrc ~/.zshrc ~/.config/fish/config.fish 2>/dev/null || true
            cleanup_files "$state_file"
...
nala_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l nala &>/dev/null; then
        if confirm "Nala detectado. Desinstalar?"; then
            ...
            dpkg -l nala &>/dev/null && sudo apt purge -y $pkg_nala
            cleanup_files "$state_file"
...
neovim_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l neovim &>/dev/null; then
        if confirm "NeoVim detectado. Desinstalar?"; then
            ...
            dpkg -l neovim &>/dev/null && sudo apt purge -y $pkg_neovim
            cleanup_files "$state_file"
...
pacstall_installer() {
    ...
    if [ -f "$state_file" ]; then
        if confirm "Pacstall detectado. Desinstalar?"; then
            ...
            sudo bash -c "$(curl -fsSL https://git.io/JcDdS)"
            cleanup_files "$state_file"
...
pessoal_base_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l fonts-jetbrains-mono &>/dev/null; then
        if confirm "Pacotes Base detectados. Desinstalar?"; then
            ...
            dpkg -l fonts-noto &>/dev/null && sudo apt purge -y $pkg_base
            cleanup_files "$state_file"
...
pessoal_media_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l gstreamer1.0-alsa &>/dev/null; then
        if confirm "Pacotes de Mídia detectados. Desinstalar?"; then
            ...
            dpkg -l ffmpeg &>/dev/null && sudo apt purge -y $pkg_media
            cleanup_files "$state_file"
...
podman_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l podman &>/dev/null; then
        if confirm "Podman detectado. Desinstalar?"; then
            ...
            dpkg -l podman &>/dev/null && sudo apt purge -y $pkg_podman
            cleanup_files "$state_file"
...
shader_booster_installer() {
    ...
    if [ -f "$state_file" ] || [ -f "$boost_file" ]; then
        ...
        if confirm "Instalar Shader Booster?"; then
            ...
            [ -n "$has_nvidia" ] && curl -fsSL https://raw.githubusercontent.com/psygreg/shader-booster/main/patch-nvidia >> "$dest_file" && patch_applied=1
            [ -n "$has_mesa" ] && curl -fsSL https://raw.githubusercontent.com/psygreg/shader-booster/main/patch-mesa >> "$dest_file" && patch_applied=1
...
snapd_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l snapd &>/dev/null; then
        if confirm "Snapd detectado. Desinstalar?"; then
            ...
            dpkg -l snapd &>/dev/null && sudo apt purge -y $pkg_snapd
            cleanup_files "$state_file"
...
starship_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l starship &>/dev/null; then
        if confirm "Starship detectado. Desinstalar?"; then
            ...
            dpkg -l starship &>/dev/null && sudo apt purge -y $pkg_starship
...
ufw_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l ufw &>/dev/null; then
        if confirm "UFW detectado. Desinstalar?"; then
            ...
            dpkg -l ufw &>/dev/null && sudo apt purge -y $pkg_ufw
...
xdg_base_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l xdg-user-dirs &>/dev/null; then
        if confirm "XDG Base detectado. Desinstalar?"; then
            ...
            dpkg -l xdg-user-dirs &>/dev/null && sudo apt purge -y $pkg_xdg
            cleanup_files "$state_file"
...
yt_dlp_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l yt-dlp &>/dev/null; then
        if confirm "yt-dlp detectado. Desinstalar?"; then
            ...
            dpkg -l yt-dlp &>/dev/null && sudo apt purge -y $pkg_ytdlp
            cleanup_files "$state_file"
...
main_menu() {
    while true; do
        clear
        echo "=== Debian Scripts ==="
        echo "1)  Gerenciar Pacotes de Sistema"
        echo "2)  Gerenciar Shells e Terminais"
        echo "3)  Gerenciar Ambientes Gráficos"
        echo "4)  Gerenciar Drivers"
        echo "5)  Gerenciar Jogos"
        echo "6)  Gerenciar Navegadores"
        echo "7)  Gerenciar Ferramentas"
        echo "8)  Gerenciar Repositórios"
        echo "9)  Sair"
        echo
        ...
        case $opcao in
            1)
                ...
                echo "11) Voltar"
                echo
                read -p "Selecione: " subopcao
                case $subopcao in
                    1) apparmor_installer ;;
                    2) archiving_compression_installer ;;
                    3) curl_installer ;;
                    4) fwupd_installer ;;
                    5) pessoal_media_installer ;;
                    6) nala_installer ;;
                    7) pessoal_base_installer ;;
                    8) podman_installer ;;
                    9) ufw_installer ;;
                    10) xdg_base_installer ;;
                esac
                ;;
            2)
                clear
                ...
                case $subopcao in
                    ...
                    3) neovim_installer ;;
                    4) lazyvim_installer ;;
                    5) starship_installer ;;
                esac
                ;;
            3)
                clear
                echo "=== Ambientes Gráficos ==="
                echo "1)  GNOME"
                echo "2)  Plasma (KDE)"
                echo "3)  Flatpak + Flathub"
                echo "4)  Snapd"
            ...
            5)
                ...
                echo "2)  Gamemode"
                echo "3)  Steam"
                echo "4)  Unmojang (Fjord)"
                echo "5)  Voltar"
                echo
                read -p "Selecione: " subopcao
                case $subopcao in
                    1) faugus_launcher_installer ;;
                    2) gamemode_installer ;;
                    3) steam_installer ;;
                    4) unmojang_installer ;;
                esac
                ;;
            6)
                clear
            ...
            8)
                clear
                echo "=== Repositórios ==="
                echo "1)  CONTRIB/NON-FREE"
                echo "2)  DebMultimedia"
                echo "3)  Voltar"
                echo
                read -p "Selecione: " subopcao
                case $subopcao in
                    1) add_contrib_nonfree_repos ;;
                    2) deb_multimedia_installer ;;
                esac
                ;;
            9) exit 0 ;;
            *) ;;
