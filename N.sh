...
EOF
    ...
    # Agora modificar conforme necessário
    if [ "$encrypt" = "true" ]; then
        local crypt_part=$(sudo blkid | grep LUKS | cut -d: -f1)
        local crypt_uuid=$(sudo blkid -s UUID -o value $crypt_part)
        sudo sed -i "s|/dev/disk/by-label/NIXROOT|/dev/mapper/cryptroot|g" /mnt/etc/nixos/hardware-configuration.nix
...
EOF
    ...
    if [ "$fs" = "btrfs" ]; then
        sudo sed -i 's|fsType = "ext4";|fsType = "btrfs";|g' /mnt/etc/nixos/hardware-configuration.nix
