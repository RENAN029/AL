...
generate_configs() {
    ...
    # Determinar o tipo de filesystem para a partição boot
    local boot_fs_type="ext4"
    if [ -d /sys/firmware/efi ]; then
        boot_fs_type="vfat"
    fi
    
    # Criar um novo hardware-configuration.nix com a sintaxe correta
...
EOF
    ...
    # Adicionar configuração LUKS se necessário (com a sintaxe correta)
...
EOF
    ...
    # Fechar o bloco corretamente
    sudo tee -a /mnt/etc/nixos/hardware-configuration.nix > /dev/null << EOF
}
EOF
    
    # Adicionar compressão btrfs se necessário (substituindo a linha options)
    ...
    if ! nix-instantiate --parse /mnt/etc/nixos/hardware-configuration.nix > /dev/null 2>&1; then
        ...
        echo "Conteúdo do arquivo com problema:"
