...
add_contrib_nonfree() {
    ...
    if [ -f "$state_file" ]; then
        if confirm "Repositórios CONTRIB e NON-FREE detectados. Remover?"; then
            ...
            sudo sed -i 's/ main non-free-firmware/ main non-free-firmware/g' /etc/apt/sources.list.d/*.list 2>/dev/null || true
            sudo apt update
...
archiving_compression_installer() {
    ...
    local pkg_compactacao="tar p7zip-full unrar unzip gzip xz-utils zip lzop"
...
deb_multimedia_installer() {
    ...
    if [ -f "$state_file" ] || [ -f "/etc/apt/sources.list.d/deb-multimedia.sources" ]; then
        ...
        if confirm "Instalar repositório DebMultimedia?"; then
            ...
            curl -L -o /tmp/deb-multimedia-keyring.deb "$keyring_url"
    ...
    if [ -f "$fish_state" ] || dpkg -l fish &>/dev/null; then
        if confirm "Fish Shell detectado. Desinstalar?"; then
            ...
            if [ -f "$fisher_state" ] || [ -d "$HOME/.config/fish/functions/fisher.fish" ]; then
                rm -rf "$HOME/.config/fish/functions/fisher.fish" 2>/dev/null || true
...
mise_installer() {
    ...
    if [ -f "$state_file" ] || command -v mise &>/dev/null; then
        if confirm "Mise detectado. Desinstalar?"; then
            ...
            rm -rf "$HOME/.local/share/mise" 2>/dev/null || true
        ...
        if confirm "Instalar Mise?"; then
            ...
            [ -f ~/.zshrc ] && echo 'eval "$(~/.local/share/mise/bin/mise activate zsh)"' >> ~/.zshrc
            [ -f ~/.config/fish/config.fish ] && echo '~/.local/share/mise/bin/mise activate fish | source' >> ~/.config/fish/config.fish
            touch "$state_file"
            echo "Mise instalado."
        fi
    fi
...
nvidia_proprietary_dkms_installer() {
    ...
    if [ -f "$state_file" ] || dpkg -l nvidia-driver &>/dev/null; then
        if confirm "Nvidia Proprietário detectado. Desinstalar?"; then
            ...
            sudo apt remove -y --purge nvidia-driver nvidia-settings nvidia-open
            cleanup_files "$state_file"
        ...
        if confirm "Instalar Nvidia Proprietário?"; then
            ...
            sudo apt update
            sudo apt install -y nvidia-driver cuda-drivers
...
starship_installer() {
    ...
    if [ -f "$state_file" ] || command -v starship &>/dev/null; then
        if confirm "Starship detectado. Desinstalar?"; then
            ...
            rm -rf "$HOME/.local/bin/starship" 2>/dev/null || true
        ...
        if confirm "Instalar Starship?"; then
            ...
            [ -f ~/.bashrc ] && echo -e "\neval \"\$(~/.local/bin/starship init bash)\"" >> ~/.bashrc
            [ -f ~/.zshrc ] && echo -e "\neval \"\$(~/.local/bin/starship init zsh)\"" >> ~/.zshrc
            command -v fish &>/dev/null && mkdir -p ~/.config/fish && if [ -f ~/.config/fish/config.fish ]; then echo -e "\n~/.local/bin/starship init fish | source" >> ~/.config/fish/config.fish; else echo -e "~/.local/bin/starship init fish | source" > ~/.config/fish/config.fish; fi
            touch "$state_file"
...
zswap_installer() {
    ...
    if [ -f "$state_file" ]; then
        if confirm "ZSWAP detectado. Desativar?"; then
            ...
            sudo sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT="/&/; s/ zswap.enabled=1//' /etc/default/grub
            sudo update-grub
            cleanup_files "$state_file"
...
main_menu() {
    while true; do
        clear
        echo "=== Debian Scripts ==="
        echo "1) Repositórios e Sistema"
        echo "2) Ambientes Desktop"
        echo "3) Desenvolvimento"
        echo "4) Multimídia e Jogos"
        echo "5) Utilitários"
        echo "6) Sair"
        echo
